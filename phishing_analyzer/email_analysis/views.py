...
class EmailAnalysisViewSet(viewsets.ModelViewSet):
    ...
    def generate_report(self, request):
        ...
        if validated_data['report_format'] == 'PDF':
            ...
            buffer = io.BytesIO()
            # --- Professional PDF with Platypus ---
            ...
            elements.append(Spacer(1, 0.1*inch))
            elements.append(Paragraph(f"Generated by: <b>{user_str}</b> on <b>{date_str}</b>", styles['Normal']))
            ...
            if not analyses_list:
                elements.append(Paragraph("No analyses found for the selected range.", styles['Normal']))
            else:
                for a in analyses_list:
                    ...
                    data.append(row)
                # Table style
                table = Table(data, colWidths=[0.6*inch, 1.5*inch, 1.5*inch, 1.5*inch, 0.7*inch, 0.7*inch, 1*inch])
